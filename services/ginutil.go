...
func (g *Gin) UploadFile() (fileUrl, filePath string, ok bool) {
	...
	fileName := fileutil.GetUploadFileName(fileHeader.Filename)
	fullPath := fileutil.GetUploadFileFullPath()
	savePath := fileutil.GetUploadFilePath()
	src := fullPath + fileName
	if !fileutil.CheckUploadFileExt(src) || !fileutil.CheckUploadFileSize(file) {
		g.ResponseCode(STATUS_CODE_UPLOAD_FILE_CHECK_FORMAT_WRONG, nil)
		return "", "", false
	}
	if err := fileutil.CheckUploadFile(fullPath); err != nil {
		logging.Warning(err)
	...
	fileUrl = fileutil.GetUploadFileFullUrl(fileName)
