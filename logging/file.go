...
func openLogFile(fileName, filePath string) (*os.File, error) {
	...
	src := filePath
	perm := fileutil.CheckPermission(src)
	if perm == true {
		return nil, fmt.Errorf("file.CheckPermission Permission denied src: %s", src)
	}

	err := fileutil.MKDirIfNotExist(src)
	if err != nil {
		return nil, fmt.Errorf("file.IsNotExistMkDir src: %s, err: %v", src, err)
	}

	f, err := fileutil.Open(src + fileName, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
...
func fileTimePassDaySlice(levelFlag string) bool {
	...
	currentLogFileExist := fileutil.CheckExist(getLogFilePath() + getLogFileName(levelFlag))
	if !currentLogFileExist {
		return false
	}
	// 昨日日志文件存在，说明已处理，不继续处理
	lastDayLogFileExist := fileutil.CheckExist(getLogFilePath() + getLastDayLogFileName(levelFlag))
